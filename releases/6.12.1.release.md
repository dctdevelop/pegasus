...
* Changed, labels for the Syrus Lynx devices are updated from: to
* * `gtstr`: tripstrt | trip start
* * `gtstp`: tripend | trip end
* * `gtbaa`: tmp | temperature and humidity alert
* * `gtver`: ver | firmware version response
* * `gtupd`: fwupdt | firmware update response
* * `gtlsp`: stp | long stop
